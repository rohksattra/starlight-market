...
def test_worker_totals_reject_when_over_capacity() -> None:
    service, users, orders = _service()
    users.get_user.return_value = {"total_worker_income": 0}
    orders.count_active_by_worker.return_value = 1
    orders.sum_active_claim_quantity_by_worker.return_value = DEFAULT_WORKER_LIMITS.claim_capacity + 1

    try:
        _run(service.validate_worker_totals(worker_id="w1"))
        raise AssertionError("expected ValueError")
    except ValueError as exc:
        assert "Claim capacity limit reached" in str(exc)
