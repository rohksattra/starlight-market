...
def test_inject_item_emoji_uses_catalog_cache() -> None:
    service = _service()
    cached = {"1": {"item_id": "1", "item_name": "Potion", "item_emoji": "🧪"}}
    with patch(
        "services.market.catalog_cache.item_map",
        AsyncMock(return_value=cached),
    ):
        rows = asyncio.run(service._inject_item_emoji([{"item_id": "1", "value": 3}]))
    assert rows[0]["name"] == "Potion"
    assert rows[0]["item_emoji"] == "🧪"
    service.items.get_all.assert_not_called()
