...
def test_claim_checks_tier_then_increments() -> None:
    ...
    tier.validate_worker_totals.assert_awaited_once_with(worker_id="w1")
...
def test_claim_rolls_back_when_post_check_fails() -> None:
    ...
    tier.validate_worker_totals.side_effect = ValueError("Claim capacity limit reached")

    try:
        _run(service.claim(order_id="o1", worker_id="w1", qty=3))
        raise AssertionError("expected ValueError")
    except ValueError as exc:
