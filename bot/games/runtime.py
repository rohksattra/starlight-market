...
class GameRuntimeService:
    ...
    async def state(
        ...
        doc = await self.game_serv(ctx).get_state(game_type=game_type)
        if not doc:
            return None
        state = doc.get("state")
    ...
    async def edit_game_panel(
        ...
        *,
        ...
        panel = await self.game_serv(ctx).get_panel(panel_type="game", game_type=game_type)
        if not panel:
            return
    ...
    async def reset_counting(self, ctx: GameContext) -> dict[str, Any]:
        ...
        await self.game_serv(ctx).upsert_state(game_type="counting", state=state)
        return state
    ...
    async def reset_wordchain(self, ctx: GameContext) -> dict[str, Any]:
        ...
        await self.game_serv(ctx).upsert_state(game_type="wordchain", state=state)
        return state

    async def reset_scramble(self, ctx: GameContext) -> dict[str, Any]:
        state = await self.game_serv(ctx).scramble_word()
        await self.game_serv(ctx).upsert_state(game_type="scramble", state=state)
        return state
    ...
    async def reset_enemy(
        ...
        *,
        ...
        await self.game_serv(ctx).upsert_state(game_type=game_type, state=state)
        return state
    ...
    async def attack_enemy(
        ...
        *,
        ...
        new_state = await self.game_serv(ctx).try_apply_battle_hit(
            game_type=game_type,
            user_id=user_id,
            dealt=dealt,
            killed=killed,
