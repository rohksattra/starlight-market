...
class GameHandler:
    ...
    async def send_leaderboard_panel(
        ...
        *,
        ...
        await self.runtime.game_serv(ctx).upsert_panel(
    ...
    async def send_game_panel(
        ...
        *,
        ...
        await runtime.game_serv(ctx).upsert_panel(
            panel_type="game",
            game_type=game_type,
            channel_id=str(channel.id),
            message_id=str(message.id),
...
class GameMessageHandler:
    ...
    async def _handle_counting_answer(
        ...
        if not await self.runtime.game_serv(ctx).try_claim_answer(
            game_type="counting",
            answer_key=answer,
        ):
            return
    ...
    async def _handle_wordchain_answer(
        ...
        await self.runtime.game_serv(ctx).upsert_state(game_type="wordchain", state=state)
        await self.runtime.game_serv(ctx).add_points(
            user_id=str(message.author.id),
            game_type="wordchain",
            score_points=1,
