...
class OrderStaffEditsMixin:
    ...
    async def handle_update_quantity(
        ...
        *,
        ...
        await channel.send(content=content, embed=embed)
        if override:
            confirm = "✅ Order item quantity overridden."
            if mode == "add":
                action = (
                    f"Overrode order quantity by +{quantity:,} ({old_qty:,} → {new_qty:,}) "
                    f"for {updated.get('item_name')} in #{channel.name}"
                )
            elif mode == "reduce":
                action = (
                ...
                action = (
                    f"Overrode order quantity from {old_qty:,} to {new_qty:,} "
                    f"for {updated.get('item_name')} in #{channel.name}"
                )
        else:
            confirm = "✅ Order item quantity updated."
            if mode == "add":
                action = (
        ...
        await safe_respond(interaction, content=confirm, ephemeral=True)
        await log_activity(
            guild=interaction.guild,
            ctx=ctx,
            member=interaction.user,
