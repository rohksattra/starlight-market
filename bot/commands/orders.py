...
class OrderCommands(commands.Cog):
    ...
    @app_commands.command(
        name="override-order-quantity",
        description="(Staff) Override order quantity without using customer limit",
    )
    @app_commands.describe(
        mode="Set a new total, or add/reduce by an amount",
        quantity="Amount to set, add, or reduce",
    ...
    async def override_quantity(
        ...
        await safe_defer(interaction, ephemeral=True)
        if not await self._require_staff_interaction(interaction, key="order_override_quantity"):
            return
        await self.handler.handle_update_quantity(
            interaction,
            mode=mode,
            quantity=quantity,
            override=True,
