...
from bot.ui.monster_grind import grind_estimate_embed
...
from services.monster_grind import MAX_KILL_COUNT, MonsterGrindService
...
class CommunityCommands(commands.Cog):
    ...
    async def monster_autocomplete(
        ...
        if tenant is None or tenant.game != "coa":
            return []
        choices = await MonsterGrindService(tenant).autocomplete(current)
        return [app_commands.Choice(name=label, value=value) for label, value in choices]

    @app_commands.command(
        name="monster-grind",
        description="Estimate average drops from grinding a monster",
    )
    @app_commands.describe(
        monster="Select a monster (type to search)",
        kill_count="Number of kills",
    )
    @app_commands.autocomplete(monster=monster_autocomplete)
    async def monster_grind(
        self,
        interaction: discord.Interaction,
        monster: str,
        kill_count: app_commands.Range[int, 1, MAX_KILL_COUNT],
    ) -> None:
        if interaction.guild is None:
        ...
        if tenant.game != "coa":
            await safe_respond(
                interaction,
                content="❌ This command is only available in Starlight Market (CoA).",
                ephemeral=True,
            )
            return

        try:
            check_cooldown(user_id=interaction.user.id, key="monster_grind", seconds=5)
        except ValueError as exc:
            await safe_respond(interaction, content=f"⏳ {exc}", ephemeral=True)
            return
