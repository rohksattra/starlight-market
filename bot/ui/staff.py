...
CID_BOOST = "sl_rc:boost"
CID_METEOR = "sl_rc:meteor"
...
def role_claim_embed(ctx: GameContext) -> discord.Embed:
    ...
    if ctx.game == "coa" and ctx.roles.world_boost:
        embed.description += (
            "\n\n### 🚀 Boost\n"
            "Pings when a world is boosted (+50% EXP)."
        )
    if ctx.game == "coa" and ctx.roles.meteor:
        embed.description += (
            "\n\n### ☄️ Meteor\n"
            "Pings 5 minutes before a meteor drop."
        )
...
def _role_id_for_custom_id(guild_id: int, custom_id: str) -> int | None:
    ...
    mapping = {
        ...
        CID_BOOST: ctx.roles.world_boost,
        CID_METEOR: ctx.roles.meteor,
    }
...
class RoleClaimView(ui.View):
    ...
    def for_context(cls, ctx: GameContext) -> RoleClaimView:
        coa = ctx.game == "coa"
        return cls(
            include_boost=coa and bool(ctx.roles.world_boost),
            include_meteor=coa and bool(ctx.roles.meteor),
        )
    ...
    @ui.button(label="Giveaway", style=discord.ButtonStyle.secondary, custom_id=CID_GIVEAWAY, row=0)
    async def btn_giveaway(self, interaction: discord.Interaction, button: ui.Button) -> None:
        await self._toggle(interaction, button.custom_id)

    @ui.button(label="Content", style=discord.ButtonStyle.secondary, custom_id=CID_CONTENT, row=0)
    async def btn_content(self, interaction: discord.Interaction, button: ui.Button) -> None:
        await self._toggle(interaction, button.custom_id)

    @ui.button(label="Boost", style=discord.ButtonStyle.secondary, custom_id=CID_BOOST, row=1)
    async def btn_boost(self, interaction: discord.Interaction, button: ui.Button) -> None:
        await self._toggle(interaction, button.custom_id)

    @ui.button(label="Meteor", style=discord.ButtonStyle.secondary, custom_id=CID_METEOR, row=1)
    async def btn_meteor(self, interaction: discord.Interaction, button: ui.Button) -> None:
        await self._toggle(interaction, button.custom_id)
