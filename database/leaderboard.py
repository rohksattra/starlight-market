"""Mongo queries for market leaderboards."""
...
class LeaderboardRepo:
    ...
    async def top_items(self, *, limit: int = 100) -> list[LeaderboardRow]:
        cursor = (
            self.items.find(
                {"item_sold": {"$gt": 0}},
                {"_id": 0, "item_id": 1, "item_name": 1, "item_sold": 1},
            )
            .sort("item_sold", -1)
            .limit(limit)
        )
    ...
    async def top_game(self, *, game_type: str, limit: int = 100) -> list[LeaderboardRow]:
        from models.games import GAME_SCORE_FIELDS

        field = GAME_SCORE_FIELDS.get(game_type)  # type: ignore[arg-type]
        if field is None:
        ...
        cursor = (
            self.users.find(
                {field: {"$gt": 0}},
                {"_id": 0, "user_id": 1, field: 1},
            )
            .sort(field, -1)
            .limit(limit)
        )
        return [
            {"id": d["user_id"], "value": int(d.get(field, 0))}
            async for d in cursor
        ]
