...
log = logging.getLogger("database.indexes")
...
async def ensure_indexes(db_name: str) -> None:
    ...
    if not await _has_index(db.items, keys=["item_sold"]):
        await db.items.create_index([("item_sold", -1)])
    ...
    if not await _has_index(db.orders, keys=["order_status", "order_claims.order_claimable"]):
        await db.orders.create_index(
            [("order_status", 1), ("order_claims.order_claimable", -1)]
        )
