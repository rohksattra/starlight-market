...
class OrderRepo:
    ...
    async def set_channel(self, order_id: str, channel_id: str) -> None:
        await self.orders.update_one(
            {"order_id": order_id},
            {"$set": {"channel_id": channel_id, "updated_at": utc_now()}},
        )

    async def set_embed_message(self, order_id: str, message_id: str) -> None:
        await self.orders.update_one(
            {"order_id": order_id},
            {"$set": {"embed_message_id": message_id, "updated_at": utc_now()}},
        )
    ...
    async def sum_active_quantity_by_customer(self, customer_id: str) -> int:
        cursor = self.orders.find(
            ...
            {"item_quantity": 1, "quota_quantity": 1, "_id": 0},
        )
        total = 0
        async for doc in cursor:
            raw = doc.get("quota_quantity")
            if raw is None or raw == "":
                total += int(doc.get("item_quantity", 0) or 0)
            else:
                total += int(raw)
        return total
    ...
    async def inc_claim(self, *, order_id: str, worker_id: str, qty: int) -> Order | None:
        return await self.orders.find_one_and_update(
            ...
            },
            {
                ...
                "$set": {"order_status": OrderStatus.CLAIMED, "updated_at": utc_now()},
            },
            projection={"_id": 0},
            return_document=ReturnDocument.AFTER,
        )
    ...
    async def delete_by_status_updated_before(
        self,
        *,
        statuses: list[str],
        cutoff: datetime,
