...
class StaffCommands(commands.Cog):
    ...
    async def category_autocomplete(
        ...
        return [
            ...
            for c in await ItemService(ctx).list_categories()
            if query in c.lower()
        ][:25]
    ...
    async def item_by_category_autocomplete(
        ...
        if not category:
            return []
        items = await ItemService(ctx).search_items(current, limit=25, category=str(category))
        return [
    ...
    async def roles_command(self, ctx: commands.Context) -> None:
        ...
        try:
            await channel.send(embed=role_claim_embed(tenant), view=RoleClaimView.for_context(tenant))
