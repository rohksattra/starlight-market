"""Worker rating request and submit (GameContext-aware)."""
...
log = logging.getLogger("services.ratings")


class WorkerRatingService:
    def __init__(self, ctx: GameContext) -> None:
        self.ctx = ctx
        self.worker_ratings = WorkerRatingRepo(ctx.db_name)
    ...
    async def request_rating(
        ...
        *,
        ...
        log.info(
            "Worker rating requested | transaction=%s worker=%s customer=%s",
            transaction_id,
            worker_id,
            customer_id,
        )
    ...
    async def submit_rating(
        ...
        *,
        ...
        if record["customer_id"] != customer_id:
            log.warning(
                "Rating denied | not owner | transaction=%s user=%s",
                transaction_id,
                customer_id,
            )
        ...
        ok = await self.worker_ratings.rating_submit(
            transaction_id=transaction_id,
            rating=rating,
            rated_at=now,
        )
        ...
        await self.users.ensure_user(worker_id)
        await self.users.inc_worker_rating(user_id=worker_id, rating=rating)
        log.info(
            "Worker rated | transaction=%s worker=%s rating=%s",
            transaction_id,
            worker_id,
            rating,
        )
