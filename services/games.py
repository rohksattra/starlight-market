...
class GameService:
    def __init__(self, ctx: GameContext) -> None:
        ...
        self.games = GameRepo(ctx.db_name)

    async def get_state(self, *, game_type: GameType) -> dict[str, Any] | None:
        return await self.games.get_state(game_type=game_type)

    async def get_panel(self, *, panel_type: GamePanelType, game_type: GameType):
        return await self.games.get_panel(panel_type=panel_type, game_type=game_type)
    ...
    async def try_claim_answer(
        ...
        *,
        ...
        return await self.games.try_claim_answer(
            game_type=game_type,
    ...
    async def try_apply_battle_hit(
        ...
        *,
        ...
        return await self.games.try_apply_battle_hit(
            game_type=game_type,
            user_id=user_id,
            dealt=dealt,
            killed=killed,
    ...
    async def _scramble_pool(self) -> list[dict[str, str]]:
        ...
        for item in await catalog_cache.items(self.ctx.db_name):
        ...
        for monster in await catalog_cache.monsters(self.ctx.db_name):
    ...
    def battle_pool(
        ...
        *,
        ...
        typed = [row for row in ready if str(row.get("monster_type") or "").strip()]
        if typed:
            wanted = {"boss", "minion"} if game_type == "boss" else {"normal"}
            return [
                row
                for row in typed
                if str(row.get("monster_type") or "").strip().lower() in wanted
            ]
        if game_type == "boss":
            return [
                row
    ...
    def _catalog_battle_state(
        ...
        *,
        ...
        emoji = str(monster.get("monster_emoji") or "").strip()
        if not emoji:
    ...
    async def battle_state(self, game_type: PlayableGameType) -> dict[str, Any]:
        rows = [
            ...
            if int(row.get("monster_health") or 0) >= 1
        ]
