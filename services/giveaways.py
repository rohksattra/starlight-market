"""Service for giveaways — no Discord imports."""
...
from database.giveaways import GiveawayRepo
...
class GiveawayService:
    def __init__(self, ctx: GameContext) -> None:
        self.ctx = ctx
        self.repo = GiveawayRepo(ctx.db_name)
    ...
    async def set_status(self, *, giveaway_id: str, status: str) -> None:
        await self.repo.set_status(giveaway_id=giveaway_id, status=status)  # type: ignore[arg-type]
