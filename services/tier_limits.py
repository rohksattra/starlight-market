"""Tier limit validation for customers and workers."""
...
class TierLimitsService:
    ...
    async def validate_customer_order(self, *, customer_id: str, quantity: int) -> None:
        ...
        limits = customer_limits_for_spent(spent, game=self.ctx.game)
    ...
    async def validate_worker_claim(
        ...
        *,
        ...
        limits = worker_limits_for_income(income, game=self.ctx.game)
    ...
    async def validate_worker_totals(self, *, worker_id: str) -> None:
        user = await self.users.get_user(worker_id) or {}
        income = int(user.get("total_worker_income", 0) or 0)
        limits = worker_limits_for_income(income, game=self.ctx.game)
        ...
        if limits.claim_capacity is not None:
            active_qty = await self.orders.sum_active_claim_quantity_by_worker(worker_id)
            if active_qty > limits.claim_capacity:
                raise ValueError(
                    f"Claim capacity limit reached (**{active_qty:,}/{limits.claim_capacity:,}**)"
                )

    async def validate_customer_totals(self, *, customer_id: str) -> None:
        user = await self.users.get_user(customer_id) or {}
        spent = int(user.get("total_customer_spent", 0) or 0)
        limits = customer_limits_for_spent(spent, game=self.ctx.game)
        ...
        if limits.order_capacity is not None:
            active_qty = await self.orders.sum_active_quantity_by_customer(customer_id)
            if active_qty > limits.order_capacity:
                raise ValueError(
                    f"Order capacity limit reached (**{active_qty:,}/{limits.order_capacity:,}**)"
                )
    ...
    async def get_profile_limits(self, *, user_id: str) -> ProfileLimitInfo:
        ...
        game = self.ctx.game
        donor_limits = donor_limits_for_total(donation, game=game)
        worker_limits = worker_limits_for_income(income, game=game)
        customer_limits = customer_limits_for_spent(spent, game=game)

        if donor_limits.max_coupons is None:
            coupon_remaining = 0
            coupon_max: int | None = None
        else:
