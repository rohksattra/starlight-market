...
class OrderService:
    ...
    async def update_quantity(
        ...
        *,
        ...
        if qty_delta > 0 and not override:
            await self.tier_limits.validate_customer_order(
                customer_id=order["customer_id"],
                quantity=qty_delta,
            )

        new_quota = next_quota_quantity(
            current_quota=quota_quantity_of(order),
            new_item_qty=new_quantity,
            item_delta=qty_delta,
            override=override,
        )
        updated = await self.orders.update_fields(
            order_id=order["order_id"],
    ...
    async def update_customer(self, *, order: Order, new_customer_id: str) -> Order:
        ...
        if order["order_status"] in {
            ...
            from services.order_claim_math import quota_quantity_of

            await self.tier_limits.validate_customer_order(
                customer_id=new_customer_id,
                quantity=quota_quantity_of(order),
            )
