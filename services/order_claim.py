...
class OrderClaimService:
    ...
    async def _claim_with_limits(
        ...
        *,
        ...
        async with user_action_locks.lock((self.ctx.db_name, "worker", worker_id)):
            await self.tier_limits.validate_worker_claim(
                worker_id=worker_id,
                order_id=order_id,
                quantity=qty,
            )
            order = await self._claim_base(
                order_id=order_id,
                worker_id=worker_id,
                qty=qty,
                action=action,
            ...
            try:
                await self.tier_limits.validate_worker_totals(worker_id=worker_id)
            except ValueError:
                log.warning(
                    "Rolling back over-limit claim | order=%s worker=%s qty=%s",
                    order_id,
                    worker_id,
                    qty,
                )
                await self._unclaim_base(
                    order_id=order_id,
                    worker_id=worker_id,
                    qty=qty,
                    action="Rollback",
    ...
    async def claim(self, *, order_id: str, worker_id: str, qty: int) -> dict[str, Any]:
        ...
        return await self._claim_with_limits(
            order_id=order_id,
            worker_id=worker_id,
            qty=qty,
            action="Claim",
