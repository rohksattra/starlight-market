...
class GiveawayWinnerView(ui.View):
    ...
    async def _on_reroll_all(self, interaction: discord.Interaction) -> None:
        ...
        await get_giveaway_handler().handle_reroll_all(interaction, self.giveaway_id)

    async def _on_reroll_partial(self, interaction: discord.Interaction) -> None:
        from app.handlers.giveaway import get_giveaway_handler

        await get_giveaway_handler().handle_reroll_partial_prompt(interaction, self.giveaway_id)

    async def _on_claim(self, interaction: discord.Interaction) -> None:
        from app.handlers.giveaway import get_giveaway_handler

        await get_giveaway_handler().handle_mark_claimed_prompt(interaction, self.giveaway_id)

    async def _on_close(self, interaction: discord.Interaction) -> None:
        from app.handlers.giveaway import get_giveaway_handler

        await get_giveaway_handler().handle_close_giveaway(interaction, self.giveaway_id)
