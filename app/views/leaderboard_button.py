...
class LeaderboardPaginationView(discord.ui.View):
    ...
    async def _fetch_entries(self, interaction: discord.Interaction) -> list[dict]:
        ...
        handler = get_leaderboard_handler()

        if self.lb_type == "worker":
            return (await handler.fetch_worker(guild))[:MAX_ITEMS]
        if self.lb_type == "customer":
            return (await handler.fetch_customer(guild))[:MAX_ITEMS]
        if self.lb_type == "donor":
            return (await handler.fetch_donor(guild))[:MAX_ITEMS]
        return (await handler.fetch_item())[:MAX_ITEMS]
