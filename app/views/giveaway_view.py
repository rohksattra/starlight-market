...
class GiveawayView(ui.View):
    ...
    async def _on_join(self, interaction: discord.Interaction) -> None:
        ...
        await get_giveaway_handler().handle_join(interaction, self.giveaway_id)

    async def _on_participants(self, interaction: discord.Interaction) -> None:
        from app.handlers.giveaway import get_giveaway_handler

        await get_giveaway_handler().handle_participants(interaction, self.giveaway_id)

    async def _on_refresh(self, interaction: discord.Interaction) -> None:
        from app.handlers.giveaway import get_giveaway_handler

        await get_giveaway_handler().handle_refresh(interaction, self.giveaway_id)

    async def _on_cancel(self, interaction: discord.Interaction) -> None:
        from app.handlers.giveaway import get_giveaway_handler

        await get_giveaway_handler().handle_cancel_giveaway(interaction, self.giveaway_id)