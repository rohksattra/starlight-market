...
class GameRuntimeService:
    ...
    async def state(self, game_type: GameType) -> Dict[str, Any] | None:
        doc = await self.games.get_state(game_type=game_type)
        if not doc:
            return None
        state = doc.get("state")
    ...
    async def edit_game_panel(
        ...
        *,
        ...
        panel = await self.games.get_panel(panel_type="game", game_type=game_type)
        if not panel:
            return
    ...
    async def reset_counting(self) -> Dict[str, Any]:
        ...
        await self.games.upsert_state(game_type="counting", state=state)
        return state

    async def reset_wordchain(self) -> Dict[str, Any]:
        word = self.game_serv.wordchain_seed()
        state = {"word": word, "used_words": [word], "last_user_id": None}
        await self.games.upsert_state(game_type="wordchain", state=state)
        return state

    async def reset_trivia(self) -> Dict[str, Any]:
        state = self.game_serv.trivia_question()
        await self.games.upsert_state(game_type="trivia", state=state)
        return state

    async def reset_guess(self) -> Dict[str, Any]:
        state = {"answer": self.game_serv.guess_number(), "active": True}
        await self.games.upsert_state(game_type="guess", state=state)
        return state

    async def reset_scramble(self) -> Dict[str, Any]:
        state = self.game_serv.scramble_word()
        await self.games.upsert_state(game_type="scramble", state=state)
        return state

    async def reset_reaction(self) -> Dict[str, Any]:
        state = self.game_serv.reaction_round()
        await self.games.upsert_state(game_type="reaction", state=state)
        return state
    ...
    async def schedule_reaction_auto_reset(self, *, delay_seconds: int) -> None:
        await self.cancel_reaction_auto_reset()
    ...
    async def recover_reaction_auto_reset(self) -> None:
        ...
        if not isinstance(auto_reset_at, str):
            await self.schedule_reaction_auto_reset(delay_seconds=REACTION_AUTO_RESET_SECONDS)
            return
        try:
        ...
        if remaining <= 0:
            await self.start_reaction_new_round()
            return
        await self.schedule_reaction_auto_reset(delay_seconds=int(remaining))

    async def reset_enemy(self, *, game_type: PlayableGameType) -> Dict[str, Any]:
        state = self.game_serv.boss_state() if game_type == "boss" else self.game_serv.monster_state()
        await self.games.upsert_state(game_type=game_type, state=state)
        return state

    async def cancel_battle_auto_reset(self, game_type: PlayableGameType) -> None:
    ...
    async def start_battle_new_enemy(self, game_type: PlayableGameType) -> Dict[str, Any]:
        if game_type not in BATTLE_GAME_TYPES:
            raise ValueError("Invalid battle game type")
        await self.cancel_battle_auto_reset(game_type)
        state = await self.reset_enemy(game_type=game_type)
        await self.edit_game_panel(
            game_type=game_type,
    ...
    async def schedule_battle_auto_new_enemy(self, *, game_type: PlayableGameType, delay_seconds: int) -> None:
        if game_type not in BATTLE_GAME_TYPES:
            return
        await self.cancel_battle_auto_reset(game_type)
        ...
        async def _worker() -> None:
            try:
                ...
                log.exception("Battle auto new enemy failed | game=%s", game_type)

        self._battle_auto_reset_tasks[game_type] = asyncio.create_task(_worker())

    async def recover_battle_auto_reset(self, game_type: PlayableGameType) -> None:
        if game_type not in BATTLE_GAME_TYPES:
            return
        ...
        if not isinstance(auto_new_enemy_at, str):
            await self.schedule_battle_auto_new_enemy(game_type=game_type, delay_seconds=delay_seconds)
            return
        try:
        ...
        if remaining <= 0:
            await self.start_battle_new_enemy(game_type)
            return
        await self.schedule_battle_auto_new_enemy(game_type=game_type, delay_seconds=int(remaining))

    async def claim_treasure(self, *, user_id: str) -> Dict[str, Any]:
    ...
    async def claim_reaction(self, *, user_id: str) -> Dict[str, Any]:
        if not await self.state("reaction"):
            await self.reset_reaction()
        claimed_result = await self.games.try_claim_reaction_slot(user_id=user_id)
        ...
        rewards = list(state.get("rewards") or [20, 10, 5])
        reward = int(rewards[rank - 1])
        if rank >= 3:
            reset_at = datetime.now(timezone.utc) + timedelta(seconds=REACTION_AUTO_RESET_SECONDS)
            await self.games.update_state_fields(
                game_type="reaction",
                fields={"auto_reset_at": reset_at.isoformat()},
            )
        await self.game_serv.add_points(
            user_id=user_id,
            game_type="reaction",
        ...
        if rank >= 3:
            await self.schedule_reaction_auto_reset(delay_seconds=REACTION_AUTO_RESET_SECONDS)
        return {"rank": rank, "points": reward, "claimed_count": rank}

    async def claim_daily(self, *, user_id: str) -> Dict[str, Any]:
        return await self.game_serv.claim_daily(user_id=user_id)

    async def attack_enemy(self, *, game_type: PlayableGameType, user_id: str) -> Dict[str, Any]:
        state = await self.state(game_type)
        if not state:
            state = await self.reset_enemy(game_type=game_type)
        hp = int(state.get("hp", 0) or 0)
        if hp <= 0 or not state.get("alive", True):
            ...
            if game_type in BATTLE_AUTO_NEW_ENEMY_SECONDS:
                ...
                wait = f"in **{seconds // 60} minute(s)**" if seconds >= 60 else f"in **{seconds} seconds**"
            return {
                "state": state,
                "message": f"❌ This enemy has been defeated. A new enemy will appear automatically {wait}.",
            }
        if game_type == "boss":
        ...
        if killed and game_type in BATTLE_AUTO_NEW_ENEMY_SECONDS:
            ...
            spawn_at_iso = (datetime.now(timezone.utc) + timedelta(seconds=delay)).isoformat()
        ...
        if new_state is None:
            ...
            return {"state": state, "message": "❌ Attack failed due to conflict. Please try again."}
        message = f"⚔️ You dealt **{dealt:,} damage** and gained **{sp} SP**."
        await self.game_serv.add_points(
            user_id=user_id, game_type=game_type, score_points=dealt, starlight_points=sp,
        )
        if killed:
            await self.game_serv.add_points(
                user_id=user_id, game_type=game_type, score_points=0, starlight_points=kill_bonus,
            )
            message += f"\n🏆 Last hit bonus: **{kill_bonus} SP**."
            if game_type in BATTLE_AUTO_NEW_ENEMY_SECONDS:
                delay = BATTLE_AUTO_NEW_ENEMY_SECONDS[game_type]
                wait = f"**{delay // 60} minute(s)**" if delay >= 60 else f"**{delay} seconds**"
                message += f"\n🏁 New enemy automatically in {wait}."
                await self.schedule_battle_auto_new_enemy(game_type=game_type, delay_seconds=delay)
        return {"state": new_state, "message": message}
