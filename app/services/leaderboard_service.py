...
class LeaderboardService:
    ...
    async def top_rated_workers(self, *, limit: int = 100, min_count: int = 1) -> List[Dict[str, Any]]:
        return await self.leaderboards.top_rated_workers(limit=limit, min_count=min_count)

    async def top_counting_scores(self, *, limit: int = 100) -> List[Dict[str, Any]]:
        return await self.leaderboards.top_counting_scores(limit=limit)