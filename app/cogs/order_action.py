...
class OrderActions(commands.Cog):
    ...
    async def handle_claim_refresh(self, interaction: discord.Interaction) -> None:
        ...
        if not order:
            return
        await update_order_embed(channel=interaction.channel, order=order, worker_role_id=settings.WORKER_ROLE_ID)

    async def handle_claim_action(self, interaction: discord.Interaction, *, action: Literal["claim", "unclaim"], quantity: int) -> None:
