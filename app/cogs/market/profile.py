...
from app.views.profile_embed import profile_embed
...
class Profile(commands.Cog):
    ...
    async def _send_profile(self, ctx_or_interaction: commands.Context | discord.Interaction, member: discord.Member) -> None:
        ...
        if isinstance(ctx_or_interaction, commands.Context):
            ...
            channel = guild.get_channel(settings.USER_PROFILE_CHANNEL_ID) if guild else None
        ...
        if isinstance(channel, discord.TextChannel):
            ...
            await safe_respond(ctx_or_interaction, content=f"✅ Profile sent to {channel.mention}.", ephemeral=True)
            return
