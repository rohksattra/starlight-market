...
class Leaderboard(commands.Cog):
    ...
    @commands.command(name="lbd")
    async def leaderboard_donor(self, ctx: commands.Context) -> None:
        guild = await self._validate_ctx(ctx)
        if guild is None:
            return

        channel = self._get_channel(guild, "donor")
        if channel is None:
            await ctx.send("❌ Donor leaderboard channel not found.", delete_after=5)
            await failed(ctx)
            return

        entries = await self._fetch_donor()
    ...
    @commands.command(name="rated")
    async def leaderboard_rated(self, ctx: commands.Context) -> None:
        guild = await self._validate_ctx(ctx)
        if guild is None:
            return

        channel = guild.get_channel(settings.TOP_RATED_WORKER_CHANNEL_ID)
        if not isinstance(channel, discord.TextChannel):
            await ctx.send("❌ Top rated workers channel not found.", delete_after=5)
            await failed(ctx)
            return

        from app.uis.rated_leaderboard_button import RatedLeaderboardPaginationView
        from app.uis.rated_leaderboard_embed import rated_leaderboard_embed

        entries = await self._fetch_rated_workers()
