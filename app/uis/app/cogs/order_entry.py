...
class OrderEntry(commands.Cog):
    ...
    async def start_order(self, interaction: discord.Interaction) -> None:
        ...
        async def on_category(inter: discord.Interaction, category: str) -> None:
            ...
            async def on_item(inter2: discord.Interaction, item_id: str) -> None:
                ...
                async def on_quantity(inter3: discord.Interaction, qty: int) -> None:
                    ...
                    async def on_confirm(inter4: discord.Interaction) -> None:
                        ...
                        try:
                            order = await self.order_serv.create_order(customer_id=str(inter4.user.id), item_id=item["item_id"], quantity=qty)
                        except ValueError as exc:
                        ...
                        channel = await guild.create_text_channel(name=f"【{order['order_number']}-📦】{safe_name}", category=category_channel)
                        content, embed = order_embed(order=order, worker_role_id=settings.WORKER_ROLE_ID, guild=guild)
                        ...
                        await safe_respond(inter4, content=(
                                "✅ **Order Created**\n\n"
                    ...
                    await safe_respond(inter3, content=(
                            "📝 **Confirm Order**\n\n"
            ...
            await safe_respond(inter, content="📦 Select item:", view=OrderItemView(
                    user_id=inter.user.id, items=items, page=0, page_size=20, on_pick=on_item
                ), ephemeral=True)
        await safe_respond(interaction, content="📂 Select category:", view=OrderCategoryView(
                user_id=interaction.user.id, categories=categories, page=0, page_size=20, on_select=on_category,
            ), ephemeral=True)
    ...
    async def order_entry_panel(self, ctx: commands.Context) -> None:
        ...
        await channel.send(embed=order_entry_embed(role_mention), view=OrderEntryView(self.start_order))
        await success(ctx)
