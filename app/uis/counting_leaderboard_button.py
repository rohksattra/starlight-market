...
class CountingLeaderboardPaginationView(discord.ui.View):
    def __init__(self, *, page: int = 0) -> None:
        super().__init__(timeout=None)
        self.page = page
        self._cooldowns: Dict[int, float] = {}

        prefix = "leaderboard:counting"
    ...
    async def _fetch_entries(self, interaction: discord.Interaction) -> list[dict]:
        ...
        if cog is None:
            raise RuntimeError("Counting cog missing")
        cog = cast(Any, cog)
        return (await cog._fetch_counting_top())[:MAX_ITEMS]
    ...
    async def refresh(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            await safe_respond(
                interaction,
                content="❌ Failed to refresh counting leaderboard.",
                ephemeral=True,
            )
