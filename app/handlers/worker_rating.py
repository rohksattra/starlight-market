...
class WorkerRatingHandler:
    ...
    async def handle_rating(self, interaction: discord.Interaction, *, rating: int) -> None:
        ...
        try:
            await self.worker_rating_serv.submit_rating(
...
_handler: WorkerRatingHandler | None = None


def get_worker_rating_handler() -> WorkerRatingHandler:
    global _handler
    if _handler is None:
        _handler = WorkerRatingHandler()
    return _handler
