...
from app.uis.market_statistic_button import MarketStatisticRefreshView
...
from app.uis.order_close_view import OrderCloseView
...
class StarlightBot(commands.Bot):
    async def setup_hook(self) -> None:
        ...
        self.add_view(MarketStatisticRefreshView())

        self.add_view(RatedLeaderboardPaginationView())

        self.add_view(OrderClaimView())

        self.add_view(OrderCloseView())

        self.add_view(RoleClaimView())
        ...
        gv = get_giveaway_service()
        await gv.register_persistent_views(self)
        await gv.recover_stale_giveaways(self)
