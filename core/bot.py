...
from core.tenant import GameContext, all_contexts, load_all_tenants
from core.view_registry import register_persistent_views
from core.web import start_health_server
from database.connection import close_mongo
from utils.logger import setup_logging

log = logging.getLogger("core.bot")

COA_ONLY_COMMANDS: tuple[str, ...] = ("monster-grind",)
    ...
    async def setup_hook(self) -> None:
        ...
        if not getattr(self, "_synced", False):
            for ctx in all_contexts():
                apply_guild_command_scope(self.tree, ctx)
                await self.tree.sync(guild=discord.Object(id=ctx.guild_id))
                log.info("Slash commands synced | guild=%s game=%s", ctx.guild_id, ctx.game)
            self._synced = True


def apply_guild_command_scope(tree: discord.app_commands.CommandTree, ctx: GameContext) -> None:
    guild = discord.Object(id=ctx.guild_id)
    tree.copy_global_to(guild=guild)
    if ctx.game == "coa":
        return
    for name in COA_ONLY_COMMANDS:
        ...
        for sig in (signal.SIGINT, signal.SIGTERM):
            try:
                ...
                try:
                    signal.signal(
                        sig,
                        lambda _s, _f, s=sig: loop.call_soon_threadsafe(
                            lambda: asyncio.create_task(shutdown(bot, s))
                        ),
                    )
                except (ValueError, OSError):
                    log.warning("Could not install signal handler: %s", sig.name)
